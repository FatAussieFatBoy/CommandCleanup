		...
		if (message.channel.permissionsFor(message.member).has('MANAGE_MESSAGES')) {
			...
			switch(args[0]) {
				case 'commands': //all messages that begin with the most common symbols used in commands
					...
					.then(messages => {
						let msgs = messages.filter(msg => symbols.test(msg.content.substring(0, 2)) && msg.id != message.id && message.createdTimestamp >= date_limit.getTime())
				...
				case 'bots': //all messages that are posted by bots
					message.channel.fetchMessages({ limit: 100 })
					.then(messages => {
						let msgs = messages.filter(msg => msg.author.bot && msg.pinned == false && msg.id != message.id && message.createdTimestamp >= date_limit.getTime())
				...
				case 'all': //all past 100 messages
					message.channel.fetchMessages({ limit: 100 })
					.then(messages => {
						let msgs = messages.filter(msg => msg.pinned == false && msg.id != message.id && message.createdTimestamp >= date_limit.getTime())
				...
				case 'links': //all messages that start with http or https
					message.channel.fetchMessages({ limit: 100 })
					.then(messages => {
						let msgs = messages.filter(msg => msg.content.includes('http://') || msg.content.includes('https://') && msg.pinned == false && msg.id != message.id && message.createdTimestamp >= date_limit.getTime())
				...
				case 'attachments': //all messages with attachments
					message.channel.fetchMessages({ limit: 100 })
					.then(messages => {
						let msgs = messages.filter(msg => msg.attachments.size > 0 && msg.pinned == false && msg.id != message.id && message.createdTimestamp >= date_limit.getTime())
				...
				case 'text': //all messages without attachments or links
					message.channel.fetchMessages({ limit: 100 })
					.then(messages => {
						let msgs = messages.filter(msg => msg.attachments.size === 0 && !msg.content.includes('https://') && msg.pinned == false && !msg.content.includes('http://') && msg.id != message.id && message.createdTimestamp >= date_limit.getTime())
					...
					if (message.mentions) {
						...
						if(mentioned_users) {
							mentioned_users.forEach((user, index) => {
								message.channel.fetchMessages({ limit: 100 })
									.then(messages => {
										let msgs = messages.filter(msg => msg.author.id === user.id && msg.pinned == false && msg.id != message.id && message.createdTimestamp >= date_limit.getTime())
						...
						if(mentioned_roles) {
							mentioned_roles.forEach((role, index) => {
								message.channel.fetchMessages({ limit: 100 })
									.then(messages => {
										let msgs = messages.filter(msg => msg.member.roles.exists('id', role.id) && msg.pinned == false && msg.id != message.id && message.createdTimestamp >= date_limit.getTime())
