...
module.exports.run = (client, prefix, message, args, con, dbl) => {
	...
	con.query(`SELECT * FROM guilds WHERE id = '${guild.id}'`, (err, rows) => {
		...
		if(rows) {
			...
			if(rows.length < 1) {
				sql = `INSERT INTO guilds (name, id, region, messages_deleted) VALUES ('${con.escapeId(guild.name.replace('\'', ''))}', '${guild.id}', '${guild.region}', ${msgCount})`
				console.log(`Database table for guild ${guild.name} created`)
			} else {
				let messages_deleted = rows[0].messages_deleted
				sql = `UPDATE guilds SET messages_deleted = ${messages_deleted + msgCount}, name = '${con.escapeId(guild.name.replace('\'', ''))}', region = '${guild.region}' WHERE id = '${guild.id}'`
